...
require_once'../../../codes/rector/area.php';
 ...
 $area=new area();
 $areras_unico=$area->areras_unico3();
         ...
         <br> 
          <div class="col-md-3"> 
            ...
            <div class="box box-primary" style="background-color: #fff;  box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);">
              ...
              <div class= "  table-responsive" id="pdf" style="padding: 7px;">
                ...
                <select class="form-control"   id='ss'  >  
                  ...
                  foreach ($areras_unico as $value) {
                    ...
                    if (($value['codigo']==01 or $value['area']!=1) &&( $value['id_area']!=$id_area)) {
                     
                     ?><option value="<?php echo $value['id_area'].','.$value['nombre'].','.$value['id_docente'] ?>"><?php echo $value['nombre'] ?></option><?php
                    }
                    $id_area= $value['id_area'];
                  } ?>
                </select><br> 
                <button type="button" class="btn btn-block btn-primary" style="margin-bottom: 7px;  " onclick=' var valor= document.getElementById("ss").value; buscar(valor)'>Buscar</button>
                 </div>
            </div>
          </div> 
           ...
           <div id="sapo">  </div>
                                ...
                                <form name="formulario-envia2" id="formulario-envia2" enctype="multipart/form-data" method="post">
                                  <div class="modal-body table-responsive" id="pdf">
                                    ...
                                    <strong>Actualizar Descripción:</strong><br>
                                      ...
                                      <input type="hidden" value="0" name='rol3'> 
            ...
            <div  class="box box-info" id="vv" style="display: none; background-color: #fff;  box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);"><strong>
               <div id="titulo2" style="margin-top: 7px;margin-left: 10px ;"></div><br>
                  ...
                  <button type="button" id="boton" data-toggle="modal" data-target="#mymodal" style="margin-left: 10px"   class="btn   btn-info" style="<?php echo $_POST['sty'] ?>">Nuevo</button>
...
$('.select2').select2({    
 ...
 function  buscar(va){  
  ...
  document.getElementById('id_area').value=va;
  var va1=porciones[1];
  var va2=porciones[2];
  ...
  $('#titulo').html('Plan De Area: '+va1);
  $('#titulo2').html('Plan De Area: '+va1);
  $('#titulo3').html('VER formato del Plan De Area: '+va1);
  $('#titulo4').html('Actualizar el  Plan De Area: '+va1);
  var sty='';
    $.ajax({   
        type: "post",
        data:{va1,va2,sty,id_docente},  
        url:"../../../ajax/rector/plan_area/plan_area.php?action=ver2",
        dataType:"text", 
