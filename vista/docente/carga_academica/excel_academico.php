...
include "../../../codes/rector/conexion.php"; 
    ...
    $ano=date('Y');
    $ano=date('Y');
    $tipo_calificaiones="SELECT   alumnos.nombre, alumnos.apellido,  materia_por_periodo.nota,materia_por_periodo.id_materia_por_periodo FROM alumnos, informeacademico ,materia_por_periodo WHERE alumnos.id_alumnos=informeacademico.id_alumno and informeacademico.id_curso='".$_GET['id_c']."'  AND informeacademico.id_grado='".$_GET['id_g']."' AND informeacademico.id_jornada_sede='".$_GET['id_jornada_sede']."' and  informeacademico.ano=$ano and materia_por_periodo.id_informe_academico=informeacademico.id_informe_academico  AND materia_por_periodo.periodo='".$_GET['p']."' AND materia_por_periodo.id_area='".$_GET['id_a']."'  order by alumnos.apellido";
...
foreach($registro as $student){
    ...
    $spreadsheet->getActiveSheet()
        ->setCellValue('A'.$row , $student['id_materia_por_periodo'])
...
$vo=$_GET['rom'];
header('Content-Disposition: attachment;filename="  Definitiva_periodo:'.$_GET['p'].'.xlsx"');
