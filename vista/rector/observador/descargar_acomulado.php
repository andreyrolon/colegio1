...
$observacion=$obser->consutar_observacion_acumulado($_GET['id_alumnos']);
   ...
   <table style="width:900px; padding-left: 25px;padding-right:  10px;padding-top: 5px">'
	...
	foreach ($observacion as   $valueq) {
		if ($valueq['periodo']!=$periodo) {
			 ...
			 $html.=' 
	        <tr class="titul2 centra">
				<td colspan="5">Observación '.$valueq['ano'] ." / ".$valueq['periodo'].'p</td>
	        </tr>';
	    }
        ...
        <tr class="titulcol centra">
				<td colspan="2" class="anchor3">Sede</td>
			<td class="anchor2">Jornada</td>
			<td class="tipo">Curso </td>
			<td   >Año </td>
			
        </tr>

        <tr class="  centra">
				<td  colspan="2"  class="anchor3"> '.$valueq['sede'].'</td>
			<td class="anchor2"> '.$valueq['jornada'].'</td>
			<td class="tipo"> '.$valueq['grado'].'-'.$valueq['curso'].'</td>
			<td> '.$valueq['ano'].'</td>
