...
function consultar_estudiante(){
    ...
    $consultar_nivel2="SELECT alumnos.nombre,alumnos.apellido,alumnos.foto,informeacademico.id_informe_academico FROM alumnos,informeacademico WHERE alumnos.id_alumnos=informeacademico.id_alumno and informeacademico.ano=$ano and informeacademico.id_grado=$grado AND informeacademico.id_curso=$curso  and informeacademico.id_jornada=$jornada   and informeacademico.id_sede=$sede";
...
<script src="../../../js/ImageSelect.jquery.js"></script><br>
 ...
 <option data-img-src="<?php echo $value['foto'] ?>" aria-disabled="true"  value="<?php echo $value['id_informe_academico'] ?>"><?php echo $value['nombre'].' '.$value['apellido'] ?></option>  
        <?php
    }echo '</select>
    <script>
 $(".my-select").chosen()({
...
function consultar_admin(){ 
  ...
  $sql="SELECT ROL,ID_ADMIN, FOTO,NOMBRE,APELLIDO,GENERO FROM `administradores` WHERE administradores.ROL='$rol'";
...
function consultar_alumno(){ 
  ...
  $jorna=$_POST['jorna'];
 $sql="SELECT alumnos.id_alumnos,alumnos.nombre,alumnos.apellido,alumnos.genero,alumnos.foto from alumnos,informeacademico WHERE informeacademico.ano='$ano'  and informeacademico.id_alumno=alumnos.id_alumnos and informeacademico.id_curso='$id_curso'  and  informeacademico.id_grado='$id_grado' and informeacademico.id_jornada_sede='$jorna'  ORDER BY alumnos.apellido  ";
  $sql1=$conexion->prepare($sql);
  $sql1->execute(array());
  $sql2=$sql1->fetchAll(); ?>
  <link rel="stylesheet" href="../../../css/chosen.css">
...
function registrar(){
  ...
  if ($docente=='') {
    ...
    $sqlq="INSERT INTO `mailbox` (`tabla_de`, `de`, `asunto`, `mensaje`, `archivo`,  `id_curso`,`id_grado`,`id_jornada_sede`,`dirigido`,`todos`) VALUES ('5', '$id', '$asunto', '$mensaje', '$ruta','$curso','$grado','$jornada1','$usuario','$todos')";
 ...
 echo "<br>";
  ...
  if ($usuario==2 and $docente=='' ) {
    ...
    $sql="INSERT INTO `mailbox_person` ( `tabla_para`, `para`, `id_mailbox`, `visto`, `habilitado`) SELECT '1', administradores.ID_ADMIN , '$recupera2' ,'0','1' from administradores WHERE administradores.ROL='Coordinador'  ";
    $sql1=$conexion->prepare($sql);
      $sql1->execute(array());
  }
  if ($usuario==3  and $docente=='') {
   
    $sql="INSERT INTO `mailbox_person` ( `tabla_para`, `para`, `id_mailbox`, `visto`, `habilitado`) SELECT '1', administradores.ID_ADMIN , '$recupera2' ,'0','1' from administradores WHERE administradores.ROL='Psicoorientadores'  ";
    $sql1=$conexion->prepare($sql);
      $sql1->execute(array());
  }
  if ($usuario==4 and $docente=='' ) {
   
    $sql="INSERT INTO `mailbox_person` ( `tabla_para`, `para`, `id_mailbox`, `visto`, `habilitado`) SELECT '1', administradores.ID_ADMIN , '$recupera2' ,'0','1' from administradores WHERE administradores.ROL='Secretarias' ";
    $sql1=$conexion->prepare($sql);
      $sql1->execute(array());
  }
  ...
  if ($usuario==5 and $docente=='' ) {
    ...
    $sql="INSERT INTO `mailbox_person` ( `tabla_para`, `para`, `id_mailbox`, `visto`, `habilitado`) SELECT '5',  docente.id_docente, '$recupera2' ,'0','1' FROM docente where docente.id_docente not in ('$id')";

    if($jornada1!=''){

      $sql="INSERT INTO `mailbox_person` ( `tabla_para`, `para`, `id_mailbox`, `visto`, `habilitado`)SELECT '5',  are_grado_sede.id__docente , '$recupera2' ,'0','1' from  grado_sede,are_grado_sede  WHERE grado_sede.id_jornada_sede=$jornada1  and are_grado_sede.id_grado_sede=grado_sede.id and are_grado_sede.id__docente not in('$id')   GROUP by are_grado_sede.id__docente  ";
    }
    echo $sql;
    $sql1=$conexion->prepare($sql);
    $sql1->execute(array());
