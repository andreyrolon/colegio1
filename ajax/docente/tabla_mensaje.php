...
function envios(){     
  ...
  $id=$_POST['id'];
   ...
   $consultar_nivel="SELECT   mailbox.id , mailbox.dirigido,    mailbox.fecha, mailbox.asunto from mailbox   WHERE mailbox.de='$id' and mailbox.tabla_de=5 AND mailbox.asunto like('%$d%')    ORDER BY `fecha`  DESC  "; 
  $consultar_nivel1=$conexion->prepare($consultar_nivel);
  $consultar_nivel1->execute(array()); 
  $registro=$consultar_nivel1->fetchAll();
  $nroProductos=$consultar_nivel1->rowCount();
