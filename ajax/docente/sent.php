...
function tabla()
 ...
 $paginaActual=1;
           ...
           $consultar_nivel="SELECT docente.nombre, docente.apellido, mailbox.* FROM mailbox INNER JOIN docente ON docente.id_docente=mailbox.para WHERE mailbox.de=$id AND docente.nombre LIKE('%$d%') ORDER BY mailbox.id DESC";
         $s=$consultar_nivel;
...
$o=0;
  ...
  $consultar_nivel="SELECT docente.nombre, docente.apellido, mailbox.* FROM mailbox INNER JOIN docente ON docente.id_docente=mailbox.para WHERE mailbox.de=$id AND docente.nombre LIKE('%$d%') ORDER BY mailbox.id DESC LIMIT $limit, $nroLotes ";
      $consultar_nivel1=$conexion->prepare($consultar_nivel);
      $consultar_nivel1->execute(array());
   ...
   echo '<br><table class="table table-striped table-condensed table-hover">';
    ?>
        <tbody>
                                            ...
                                            ?>
                                            <tr>
                                                <td><input type="checkbox" name="trash[]" value="<?php echo($key['id']); ?>"></td>
