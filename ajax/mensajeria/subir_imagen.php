...
if(isset($_FILES['upload']) && strlen($_FILES['upload']['name']) > 1) { 
    ...
    if(in_array($type, $imgset['type'])){ 
        ...
        if(isset($width) && isset($height)) { 
            ...
            if($width < $imgset['minwidth'] || $height < $imgset['minheight']){ 
                $re .= '\\n Width2 x Height = '. $width .' x '. $height .'\\n The minimum Width x Height must be: '. $imgset['minwidth']. ' x '. $imgset['minheight']; 
            } 
 
            if($_FILES['upload']['size'] > $imgset['maxsize']*2000){ 
                $re .= '\\El tamaño máximo del archivo debe ser: '. $imgset['maxsize']. ' KB.'; 
            } 
        } 
    }else{ 
    ...
    if($re == ''){ 
        if(move_uploaded_file($_FILES['upload']['tmp_name'], $uploadpath)) { 
            ...
            $url = 'http://alex123.mipropia.com/contenido/ckeditor/'. $upload_dir . $f_name; 
            $msg = F_NAME .'.'. $type .' la imagen se ha cargado al servidor: \\n- Size: '. number_format($_FILES['upload']['size']/2024, 2, '.', '') .' KB'; 
